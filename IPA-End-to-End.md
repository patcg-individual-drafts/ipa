...
There are some risks while executing IPA which we will like to call out explicitly

## Multi Party Computation Through Helper Nodes
The proposal relies on the existence of a network of non-colluding “helper nodes” that collaborate to produce aggregated information. The security properties of the protocol ensure that if a single helper diverges from the protocol and attacks it in arbitrary ways, it will fail to learn any personal information. However, if two helper nodes both defect from the protocol, and collude with each other to intentionally violate user privacy, they can potentially reconstruct cross-site browsing activity. This is a risk to user privacy.  

There are a few ways to potentially mitigate this risk such as:
Contractual agreements between the “helper nodes” and some coalition of browsers (this could resemble the CA-Browser Forum in terms of governance structure). These contracts might mandate periodic audits of the helper nodes to ensure good security practices are being followed.
Having the helper nodes run the protocol within a Trusted Execution Environment to provide “defense in depth”. This would pose a significant technical barrier to collusion.

There exist other MPC security paradigms such as a “dishonest majority” where even if two of the three helper parties colluded, that would not be enough to violate user privacy. These are, however, substantially less efficient. The team has assessed these and determined that the “honest majority” approach is the more cost-effective model, which we believe still offers a very high degree of security.

## Inability to determine provenance of reports
The parties operating the MPC cannot determine the provenance of the reports they receive. While the system is designed to operate on reports generated by user agents, received via calls to the “getEncryptedMatchKey” API, it is possible for an attacker to generate synthetic events. It’s as simple as generating a random number, secret-sharing it, and encrypting those shares towards the helper parties.

The implication of this, is that an attacker could generate a query which only contains source events from a single real user, and pad it out with a bunch of “dummy events” that are synthetically generated and known to not contribute to the measurement result. As a result, user privacy must be assured through managing a differential privacy budget. We cannot rely on “K-anonymity” to protect user privacy. The IPA proposal mitigates this risk by: 
Capping each user’s total contribution to a query result
Through the addition of random noise (proportional to this per-user cap)
And by managing a weekly privacy budget.
In this way, the IPA system limits the total amount of information that can be learned about an individual per week.

The IPA proposal currently suggests a very crude approach to differential privacy, managing it at the query level. That is, each query run by a business detracts from their total privacy budget. This is in contrast to more sophisticated approaches like user-level privacy budgeting. This crude approach actually provides a strong mitigation to this risk, since any budget spent trying to learn about a particular individual detracts from the business’s total privacy budget. As such, such a strategy cannot be scaled up to widespread tracking. Put another way, an attacker who spent their entire privacy budget attempting to track one person wouldn’t be able to track anyone else.

Another potential mitigation which we have heard suggested to us, is “query transparency”. The idea would be to publicly share statistics about how many queries each business runs each week, the number of records in each query, and the cardinality of breakdowns. The goal of doing this would be to highlight suspicious usage patterns which could be evidence of attempts to misuse the system to attempt to learn about specific individuals. 

## Unbounded Scope
Another consequence of the inability to determine the provenance of reports, is that the scope of information about users which can be brought into the MPC is not bound to the web. If a match key provider is able (and willing) to perform user-level linkage in other contexts (like in-store transactions), then they could potentially distribute encrypted match keys and bring in user activity to the MPC.

This is simultaneously an advantage and disadvantage. Advertisers might find an advantage in being able to perform attribution across a wide array of contexts in which a person is identifiable, like cross-device attribution or even stores and entirely offline interactions.

At worst, this could encourage expansion of surveillance without a strong basis for ensuring accountability. While the controls on the MPC system might limit the amount of information about any individual that can be obtained about a person, the use of match keys offers no way to contain the scope of information gathering.

The most extreme mitigation would be for user-agents to limit the set of entities who could act as a “match key provider” to just those entities who agreed to some kind of “code of conduct”. This approach has the downside of putting browser vendors in the position of acting as gatekeepers. That said, IPA already requires some kind of governance structure to handle the selection of approved “helper parties”, so perhaps that can be extended to match key providers as well.

A weaker mitigation would be to provide users with a transparency and control surface in their browser, which gave them the ability to block match key providers they did not trust, and transparency about which sites had requested an “encrypted match key” from their browser, and the origin bound to it in the authenticated encryption. This would serve as a useful proxy for “which businesses did I see ads for on each site”. This mitigation could be further enhanced by requiring any entity who wanted to run IPA queries to provide some public disclosures at the time they sign up with the MPC helper parties to run queries. This could include public commitments about what scope of data they would bring into the MPC.
Utility
